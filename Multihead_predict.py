...
def att_normolization(x,method='zscore',relu=False):
    ...
    if method == 'zscore':
      ...
      y = np.nan_to_num(y,0)
      return y
def new_one_hot(base):
    ref = np.array(["A","T","G","C"])
    encoded = list(np.array(ref == base,dtype="int64"))
    return encoded
...
#     X_left = [[new_one_hot(c.upper()) for c in gene.seqleft] for gene in gene_data]
#     X_right = [[new_one_hot(c.upper()) for c in gene.seqright] for gene in gene_data]
    #print(X_left)
    #print(X_right)
...
#                att_matrix3_all[i,index*batch_size:index*batch_size+len(batch_data[0])]= att_matrix3
        ...
        model.model.load_weights(kwargs['weights_dir']+str(0)+".h5")
        pred_y = model.model.predict([batch_data[0],batch_data[1].reshape(-1,batch_data[1].shape[1],1)])
        avg_predicted_y[index*batch_size:index*batch_size+len(batch_data[0])]+=pred_y
...
#          np.savetxt(OUTPATH + '/evaluation_roc_average_presicion.txt', np.array(roc_list+pr_list+mcc_list), delimiter=',')
         with open(OUTPATH + '/evaluation_roc_average_presicion.txt',"w") as f:
             all_str = f"roc_list:{roc_list}\npr_list:{pr_list}\nmcc_list:{mcc_list}"
             f.write(all_str)
...
if __name__ == "__main__":
    ...
    websiteoutput.close()
#python3 Multihead_predict.py --dataset "/binf-isilon/winthergrp/jwang/rbpnet/data/test.fasta" --outputpath ./Results/ --evaluate
