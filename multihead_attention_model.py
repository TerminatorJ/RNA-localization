...
class multihead_attention:
    ...
    def build_model_multihead_attention_multiscaleCNN4_covermore(self,
        ...
        # input = Input(shape=(self.max_len,4), dtype='float32')
        self.input = input
        ...
        # X_test = np.load("/binf-isilon/winthergrp/jwang/panRBPnet/pan-rbpnet/torchrbpnet/benchmark/DM3Loc/DM3Loc-master/testdata/Test_fold0_X.npy")[:3]
        # embedding_layer = nn.Embedding(num_embeddings=6,embedding_dim=len(seq_encoding_vectors[0]),_weight=torch.tensor(seq_encoding_vectors))
        # out = embedding_layer(torch.LongTensor(X_test))
        # out = out.detach().numpy()
        # input_data = out
        # # input_data = np.load("/binf-isilon/winthergrp/jwang/panRBPnet/pan-rbpnet/torchrbpnet/benchmark/DM3Loc/DM3Loc-master/testdata/Test_fold0_X.npy")
        # with tf.Session() as sess:
        #     sess.run(tf.global_variables_initializer())
        #     # input_data = 
        #     print("input_Data:", input_data)
        #     print("embedding_output:", sess.run(embedding_output, feed_dict={input: input_data}))
        ...
        with tf.name_scope('first_cnn'):
            ...
            if not add_avgpooling:
                ...
                #     sess.run(tf.global_variables_initializer())
                #     print(cnn_output1)
                #     print("first cnn_output:", sess.run(cnn_output1, feed_dict={input: input_data}))
                # with tf.Session() as sess:
                #     sess.run(tf.global_variables_initializer())
                #     print("cnn_output1:", sess.run(cnn_output1, feed_dict={input: input_data}))
        ...
        if posembed:
            ...
            print("###########################################they using the Embedding###########################")
            if posmod == 'concat':
        ...
        # print("********shape checking:", cnn_output1.shape, input_mask.shape)
        ...
        with tf.name_scope('multihead_attention'):
            ...
            # print("attention mask here")
            # self.model = Model(inputs=[input,input_mask], outputs=att1)
    ...
    def train(self, x_train, y_train, mask_label, batch_size, epochs=100,x_valid=None,y_valid=None,mask_valid=None,loadFinal=False,classweight=False,class_weights=None):
        if not self.is_built:
            ...
            return
        # print("y_train:",sess.run(y_train))
